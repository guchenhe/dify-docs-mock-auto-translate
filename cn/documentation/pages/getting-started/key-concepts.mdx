...
Dify 专为智能体应用构建而设计。在 **Studio** 中，你可以通过拖拽界面快速构建智能体工作流并将其发布为应用。你可以通过 API、Web 或作为 [MCP 服务器](/en/guides/application-publishing/publish-mcp)访问已发布的应用。Dify 提供两种主要的应用类型：工作流和对话流。在创建新应用时，你需要选择一种应用类型。

**手动改进说明：**
本段落经过人工审校，确保术语翻译的准确性和可读性。工作流（Workflow）和对话流（Chatflow）是 Dify 的核心应用类型。
...
**输入**：你可以在开始节点为你的应用最终用户指定任意数量的输入变量来填写。
...
此外，开始节点还带有一组输入变量，你可以在流程中稍后引用。对于不同的应用类型，会提供不同的变量。
...
**（仅对话流）会话变量：**这些变量是对话特定的——意味着它们在单个对话的多轮对话流运行中持续存在，因此你可以存储和访问动态信息，如待办事项列表和令牌成本。你可以通过变量分配器节点更新会话变量的值：
